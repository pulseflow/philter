defmodule Reflection.Define do
  @moduledoc false
  ...
  defmacro __using__(decorators) do
    ...
    decorator_macros =
      for {decorator, arity} <- decorators do
        arglist = Reflection.Decorate.generate_args(arity, decorator_module)
      ...
      defmacro __using__(_) do
        ...
        quote do
          ...
          if is_nil(Module.get_attribute(__MODULE__, :has_been_decorated)) do
            ...
            @on_definition {Reflection.Decorate, :on_definition}
            @before_compile {Reflection.Decorate, :before_compile}
          end
        end
      end
    end
